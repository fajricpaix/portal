<div class="list trending">
    <span>1.</span>
    <div class="caption list-content list-caption trending-caption">
        <h5>
            <a href="_detail.php">
                Lorem ipsum, dolor sit amet consectetur adipisicing elit. Lorem ipsum, dolor sit amet Lorem ipsum, dolor sit amet
            </a>
        </h5>
        <h6>
            <a href="javascript:;">
                <img src="img/icon/love.png" alt="love icon">
                Kategori Ekonomi
            </a>
        </h6>
        <ul class="list-inline">
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/like.png" alt="Like Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/comment.png" alt="Comment Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/share.png" alt="Share Icon">
                </a>
            </li>
        </ul>
    </div>
    <div class="list-content list-img">
        <a href="_detail.php">
            <img src="https://dummyimage.com/600x200/084494/fff" alt="Judul Berita">
        </a>
    </div>
</div>
<div class="list trending">
    <span>2.</span>
    <div class="caption list-content list-caption trending-caption">
        <h5>
            <a href="_detail.php">
                Lorem ipsum, dolor sit amet consectetur adipisicing elit. Lorem ipsum, dolor sit amet Lorem ipsum, dolor sit amet
            </a>
        </h5>
        <h6>
            <a href="javascript:;">
                <img src="img/icon/love.png" alt="love icon">
                Kategori Ekonomi
            </a>
        </h6>
        <ul class="list-inline">
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/like.png" alt="Like Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/comment.png" alt="Comment Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/share.png" alt="Share Icon">
                </a>
            </li>
        </ul>
    </div>
    <div class="list-content list-img">
        <a href="_detail.php">
            <img src="https://dummyimage.com/600x200/084494/fff" alt="Judul Berita">
        </a>
    </div>
</div>
<div class="list trending">
    <span>3.</span>
    <div class="caption list-content list-caption trending-caption">
        <h5>
            <a href="_detail.php">
                Lorem ipsum, dolor sit amet consectetur adipisicing elit. Lorem ipsum, dolor sit amet Lorem ipsum, dolor sit amet
            </a>
        </h5>
        <h6>
            <a href="javascript:;">
                <img src="img/icon/love.png" alt="love icon">
                Kategori Ekonomi
            </a>
        </h6>
        <ul class="list-inline">
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/like.png" alt="Like Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/comment.png" alt="Comment Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/share.png" alt="Share Icon">
                </a>
            </li>
        </ul>
    </div>
    <div class="list-content list-img">
        <a href="_detail.php">
            <img src="https://dummyimage.com/600x200/084494/fff" alt="Judul Berita">
        </a>
    </div>
</div>
<div class="list trending">
    <span>4.</span>
    <div class="caption list-content list-caption trending-caption">
        <h5>
            <a href="_detail.php">
                Lorem ipsum, dolor sit amet consectetur adipisicing elit. Lorem ipsum, dolor sit amet Lorem ipsum, dolor sit amet
            </a>
        </h5>
        <h6>
            <a href="javascript:;">
                <img src="img/icon/love.png" alt="love icon">
                Kategori Ekonomi
            </a>
        </h6>
        <ul class="list-inline">
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/like.png" alt="Like Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/comment.png" alt="Comment Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/share.png" alt="Share Icon">
                </a>
            </li>
        </ul>
    </div>
    <div class="list-content list-img">
        <a href="_detail.php">
            <img src="https://dummyimage.com/600x200/084494/fff" alt="Judul Berita">
        </a>
    </div>
</div>
<div class="list trending">
    <span>5.</span>
    <div class="caption list-content list-caption trending-caption">
        <h5>
            <a href="_detail.php">
                Lorem ipsum, dolor sit amet consectetur adipisicing elit. Lorem ipsum, dolor sit amet Lorem ipsum, dolor sit amet
            </a>
        </h5>
        <h6>
            <a href="javascript:;">
                <img src="img/icon/love.png" alt="love icon">
                Kategori Ekonomi
            </a>
        </h6>
        <ul class="list-inline">
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/like.png" alt="Like Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/comment.png" alt="Comment Icon">
                </a>
            </li>
            <li class="list-inline-item">
                <a href="javascript:;">
                    <img src="./img/icon/share.png" alt="Share Icon">
                </a>
            </li>
        </ul>
    </div>
    <div class="list-content list-img">
        <a href="_detail.php">
            <img src="https://dummyimage.com/600x200/084494/fff" alt="Judul Berita">
        </a>
    </div>
</div>
